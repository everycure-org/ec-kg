...
LEFT_CSV  = os.path.join(_DIR, "drug_sankey_left.csv")
RIGHT_CSV = os.path.join(_DIR, "drug_sankey_right.csv")
...
def load_data():
    ...
    # Single query: CTE materialises the drug list once, one scan of edges_unified
    # covers both directions via UNION ALL, nodes_unified joined for categories.
    combined = client.query("""
        WITH drugs AS (
            SELECT id FROM `mtrx-hub-dev-3of.release_v0_15_19.drug_list_nodes_normalized`
        )
        SELECT 'left'        AS side,
               n.category    AS node_type,
               COUNT(*)      AS edge_count
        FROM `mtrx-hub-dev-3of.release_v0_15_19.edges_unified` e
        JOIN drugs                                                     ON e.object  = drugs.id
        ...
        SELECT 'right'       AS side,
               ...
               COUNT(*)      AS edge_count
        FROM `mtrx-hub-dev-3of.release_v0_15_19.edges_unified` e
        JOIN drugs                                                     ON e.subject = drugs.id
...
def build_sankey(left_df, right_df):
    ...
    print(f"  {len(sources)} source types | EC Drug List | {len(targets)} target types")
    ...
    # Center block: half the figure height, centred vertically
...
def build_grouped_sankey(left_df, right_df):
    ...
    # Preserve the canonical GROUPS order (skip groups with zero flow)
    ...
    print(f"  {len(sources)} source groups | EC Drug List | {len(targets)} target groups")

    all_flows = src_flow.tolist() + tgt_flow.tolist()
    log_min   = math.log1p(min(all_flows))
    log_max   = math.log1p(max(all_flows))

    # More gap and larger min-height since only 8 bars per side
    gap     = 0.030
    min_h   = 0.080
...
def main():
    ...
    out_full = os.path.join(_DIR, "drug_sankey_full.pdf")
    fig_full.savefig(out_full, format="pdf", dpi=300)
    print(f"Saved {out_full}")

    fig_grouped = build_grouped_sankey(left_df, right_df)
    out_grouped = os.path.join(_DIR, "drug_sankey_grouped.pdf")
