...
#
# Import RCPARAMS / apply_style() instead of redefining rcParams locally —
# six scripts had each grown their own slightly-drifted copy (tick sizes of
# 2.0 in some, 2.5 in others; title fontsize split across 9/10/11) before
# this module existed. Same story for figsize: widths ranged 3.5–10in and
# heights 5.8–12in with no shared basis, so one figure (predicate_distribution's
# 10x12in) ended up taller than a US Letter page. figsize() below fixes the
# two widths that actually get used (single- and double-column) and warns on
# anything that doesn't fit a page.
